...
/*
  ...
  Modified by Rafael Diaz-Tushman <rdiaztushman@dioscouri.com>
  
*/
namespace Dsc\Admin\Lib\Acl\Adapter;
...
class Mongo extends Adapter implements AdapterInterface
{
    ...
    public function __construct($options)
    {
        ...
        if (!isset($options['dbhost'])) {
            throw new Exception("Parameter 'dbhost' is required");
        }
        
        if (!isset($options['dbname'])) {
            throw new Exception("Parameter 'dbname' is required");
        }
    ...
    {
        // in Mongo 1.2+, connections are always persistent
        $this->connection = (new \MongoClient('mongodb://'.$this->options['dbhost']))->selectDb($this->options['dbname']);
        
        return $this->connection->selectCollection($this->options[$name]);
    }
     ...
     * @throws \Phalcon\Acl\Exception
     */
    protected function insertOrUpdateAccess($roleName, $resourceName, $accessName, $action, $autoInsert=true)
    {
        /**
        ...
        if (!$exists && ( $resourceName != '*' && $accessName != '*') ) {
            if ($autoInsert) {
